...
**Switchroom's compliance:** The switchroom-mcp management server and switchroom-telegram plugin are standard MCP servers. They use the official `@modelcontextprotocol/sdk` package (confirmed in `~/code/switchroom/telegram-plugin/package.json` v1.0.0) and follow the documented MCP protocol.
