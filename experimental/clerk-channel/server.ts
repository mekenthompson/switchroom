#!/usr/bin/env bun
/**
 * clerk-channel — MCP channel plugin that connects to the clerk-telegram-daemon
...
const SOCKET_PATH = process.env.CLERK_SOCKET_PATH ?? "/tmp/clerk-telegram.sock";
...
if (!TOPIC_ID) {
  process.stderr.write(
    `clerk-channel: TELEGRAM_TOPIC_ID required (set as env var)\n`,
  );
  process.exit(1);
}
...
const mcp = new Server(
  { name: "clerk-telegram", version: "1.0.0" },
  {
    capabilities: {
      tools: {},
      experimental: {
    ...
    instructions: [
      ...
      "",
      'Messages from Telegram arrive as <channel source="clerk-telegram" chat_id="..." message_id="..." user="..." ts="...">.',
...
function processBuffer(socket: import("bun").Socket<{ buffer: string }>): void {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      process.stderr.write(`[clerk-channel:${AGENT_NAME}] bad JSON from daemon: ${line}\n`);
    }
  }
}
  ...
  switch (msg.type) {
    case "inbound": {
      ...
      void mcp
        ...
        .catch((err) => {
          process.stderr.write(`[clerk-channel:${AGENT_NAME}] failed to deliver inbound: ${err}\n`);
        });
      break;
    }
    case "result": {
    ...
    case "permission_response": {
      ...
      void mcp
        ...
        .catch((err) => {
          process.stderr.write(`[clerk-channel:${AGENT_NAME}] failed to deliver permission response: ${err}\n`);
        });
      break;
    }
    case "ack": {
      const a = msg as AckMessage;
      process.stderr.write(`[clerk-channel:${AGENT_NAME}] ack for ${a.originalType}: ok=${a.ok}\n`);
      break;
    }
    default:
      process.stderr.write(`[clerk-channel:${AGENT_NAME}] unknown message type: ${(msg as { type: string }).type}\n`);
  }
}

function sendToDaemon(msg: Record<string, unknown>): boolean {
  if (!daemonSocket) {
    process.stderr.write(`[clerk-channel:${AGENT_NAME}] not connected to daemon\n`);
    return false;
  }
...
async function connectToDaemon(): Promise<void> {
    ...
    try {
      daemonSocket = await Bun.connect<{ buffer: string }>({
        ...
        socket: {
          ...
          open(socket) {
            socket.data = { buffer: "" };
            process.stderr.write(`[clerk-channel:${AGENT_NAME}] connected to daemon\n`);
            reconnectAttempt = 0;
          ...
          close(_socket) {
            process.stderr.write(`[clerk-channel:${AGENT_NAME}] disconnected from daemon\n`);
            daemonSocket = null;
          ...
          drain(_socket) {},
          error(_socket, err) {
            process.stderr.write(`[clerk-channel:${AGENT_NAME}] socket error: ${err}\n`);
          },
        },
      });
      return; // success
      ...
      process.stderr.write(
        `[clerk-channel:${AGENT_NAME}] connect attempt ${attempt}/${maxRetries} failed, retry in ${(delay / 1000).toFixed(1)}s\n`,
      );
  ...
  process.stderr.write(`[clerk-channel:${AGENT_NAME}] failed to connect to daemon after ${maxRetries} attempts\n`);
  process.exit(1);
}
...
function scheduleReconnect(): void {
  ...
  process.stderr.write(
    `[clerk-channel:${AGENT_NAME}] scheduling reconnect in ${(delay / 1000).toFixed(1)}s (attempt ${reconnectAttempt})\n`,
  );
  setTimeout(() => {
    void connectToDaemon().catch((err) => {
      process.stderr.write(`[clerk-channel:${AGENT_NAME}] reconnect failed: ${err}\n`);
    });
  }, delay);
}
...
function shutdown(): void {
  ...
  shuttingDown = true;
  process.stderr.write(`[clerk-channel:${AGENT_NAME}] shutting down\n`);
  if (daemonSocket) {
    daemonSocket.end();
    daemonSocket = null;
  }
