#!/usr/bin/env bun
/**
 * clerk-telegram-daemon — single-process Telegram router for multi-agent topic routing.
 *
 * Polls Telegram via Grammy, listens on a Unix socket, and routes messages
 * between Telegram forum topics and connected clerk-channel MCP plugins.
 *
 * Architecture:
 *   [Grammy long-poll] → daemon → Unix socket → clerk-channel plugin → Claude Code agent
 *   [Grammy bot API]   ← daemon ← Unix socket ← clerk-channel plugin ← Claude Code agent
 */
...
const HOME = homedir();
const CLERK_DIR = join(HOME, ".clerk");

// Load daemon.env if present
const DAEMON_ENV_PATH = process.env.CLERK_DAEMON_ENV ?? join(CLERK_DIR, "daemon.env");
try {
...
if (!TOKEN) {
  process.stderr.write(
    `clerk-daemon: TELEGRAM_BOT_TOKEN required\n` +
...
const ACCESS_PATH = process.env.CLERK_ACCESS_PATH ?? join(CLERK_DIR, "daemon-access.json");
const INBOX_PATH = process.env.CLERK_INBOX_PATH ?? join(CLERK_DIR, "inbox");
const OFFSET_PATH = join(CLERK_DIR, "telegram-offset.json");

// Ensure directories exist
mkdirSync(CLERK_DIR, { recursive: true });
