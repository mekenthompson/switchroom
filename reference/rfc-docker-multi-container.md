...
### "No Docker requirement" — what's actually changing

Be honest: the original vision line meant "you don't need Docker to run switchroom." Under this RFC, Linux users still don't — the host-native install is fully supported and stays the recommended path on Linux. **Mac and Windows users effectively do need Docker**, because we never had a first-class non-Linux story without it. That's a real change, not a clarification, and we should say so.

Two paths going forward, not one:
- **Linux host install** (status quo): no Docker required. systemd units, the existing `install.sh`. This is the "no Docker requirement" promise, intact.
- **Docker install** (new): the multi-OS path. Required on Mac and Windows; available as an option on Linux for operators who prefer container isolation.

The OpenClaw wedge — "we don't wrap, proxy, or harness claude" — is unchanged and orthogonal to the delivery mechanism. The container runs the unmodified `claude` CLI against the user's OAuth, exactly as the systemd unit does today. See "Vision wording update" at the bottom for the revised text.
...
- Vision-preserving: stock `claude` CLI inside the container, official OAuth, no API-key interception, vault and approval kernel work identically inside and out.
...
Existing host-native install. User decides to switch.
...
Replace systemd with a TS supervisor process that spawns and manages every agent's claude REPL as a child process inside one container. **Rejected.** OOM in one agent kills the whole container — exactly the problem #793 names. Per-agent resource isolation impossible without cgroup gymnastics. Single-process model also forces every agent's tmux session into one tmux server, which complicates the `!` interrupt path. The "compose multiplies image overhead" worry from #793 is actually small (one image, multiple containers — pages of memory shared).
...
## Vision wording update

`reference/vision.md` currently says (paraphrasing) "no Docker requirement." That's accurate for Linux and stops being accurate for Mac/Windows under this RFC. Replace with two-paths language, not a softening:

> Switchroom runs two ways. On Linux, the host-native install needs no Docker — systemd units, an `apt`-style footprint, full stop. On Mac, Windows, and homelab boxes that aren't Linux, switchroom ships as a Docker image: one command up, multi-arch, the unmodified `claude` CLI inside the container running against the user's Anthropic OAuth. Either way, we do not wrap, proxy, or harness Claude. The container is a delivery mechanism, not a harness.

The OpenClaw comparison row in the same doc drops the "no Docker" point and gains "stock claude CLI inside the container, not a custom harness." Stronger wedge, not weaker. Be explicit in the doc about what's changing: Linux users keep the no-Docker promise; non-Linux users effectively need Docker, because we never had a first-class non-Linux story without it.
