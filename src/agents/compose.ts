...
/**
 * GHCR digest-pin map. When provided, the compose generator emits image
 * refs of the form `ghcr.io/switchroom/switchroom-<name>@sha256:<digest>`
 * instead of the default `ghcr.io/switchroom/<name>:<tag>` build-from-tag
 * shape. Pinning to digests gives operators reproducible deploys and is
 * the recommended production path — `:tag` floats, `@sha256:...` does
 * not.
 *
 * Keys are the short image names ("base" is unused in compose output but
 * accepted for symmetry with the build matrix). Missing keys fall back
 * to the default tag-based ref so a partial map is tolerated.
 */
export interface GhcrDigestMap {
  base?: string;
  agent?: string;
  broker?: string;
  kernel?: string;
...
export interface ComposeGeneratorOptions {
  ...
  /**
   * GHCR digest pins. When provided, services use
   * `ghcr.io/switchroom/switchroom-<name>@<digest>` instead of
   * `ghcr.io/switchroom/<name>:<tag>`. Pure additive — omitting this
   * preserves Phase 1a/2c behaviour byte-for-byte.
   */
  ghcrDigests?: GhcrDigestMap;
}

/**
 * Resolve the image ref for one of the four service images. If a digest
...
function resolveImageRef(
  ...
  imageTag: string,
  digests: GhcrDigestMap | undefined,
): string {
  const digest = digests?.[name];
  if (digest) {
    if (!/^sha256:[0-9a-f]{64}$/i.test(digest)) {
      throw new Error(
        `compose: ghcrDigests.${name} must be of the form "sha256:<64 hex chars>", got ${JSON.stringify(digest)}`,
      );
    }
...
export function generateCompose(opts: ComposeGeneratorOptions): string {
  ...
  const digests = opts.ghcrDigests;
  const brokerImage = resolveImageRef("broker", imageTag, digests);
  const kernelImage = resolveImageRef("kernel", imageTag, digests);
  const schedulerImage = resolveImageRef("scheduler", imageTag, digests);
  const agentImage = resolveImageRef("agent", imageTag, digests);
