...
describe('readAccountAccessToken', () => {
  it('returns the access token from credentials.json', () => {
    const home = makeAccountHome({
      'you@example.com': { accessToken: 'sk-ant-oat01-fake' },
    })
    try {
      expect(readAccountAccessToken('you@example.com', home)).toBe(
        'sk-ant-oat01-fake',
      )
    } finally {
...
describe('regression: boot-warm + delayed sync-read (v0.6.11)', () => {
  ...
  it('returns last-known data even after multiple TTL windows have elapsed', async () => {
    clearAccountQuotaCache()
    const home = makeAccountHome({
      'you@example.com': { accessToken: 'tok' },
    })
    try {
      const t0 = 1_000_000
      // Boot-warm: probe completes at t0.
      await fetchAccountQuota('you@example.com', {
        home,
        fetchImpl: (async () =>
          new Response('{}', {
            status: 200,
      ...
      const cached = getCachedAccountQuota('you@example.com', tDashboard)
