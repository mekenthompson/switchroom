...
agents:
  ...
  multi:
    topic_name: Multi
    auth:
      accounts: [you@example.com, me@kenthompson.com.au]
`;
...
describe("groupAgentsByPrimaryAccount", () => {
  ...
  it("groups a single agent with existing primary under the EXISTING primary, not the new label", () => {
    ...
    // primary-set already has you@example.com first. The fix means
    // we fan out you (preserve), not me@kenthompson (the new
    // fallback). primary-set's runtime active stays on you.
    ...
    expect(groups.size).toBe(1);
    expect(groups.get("you@example.com")).toEqual(["primary-set"]);
  ...
  it("groups multi-agent enable correctly: each agent gets its own primary", () => {
    ...
    //   - primary-set has you → you stays primary, new@ is
    //     fallback
    //   - multi already has you + me@ → you stays primary, new@
    ...
    expect(groups.get("new@example.com")).toEqual(["fresh"]);
    // Both pre-existing-primary agents share you as their primary.
    expect(groups.get("you@example.com")?.sort()).toEqual([
      "multi",
      "primary-set",
    ]);
  });
  ...
  it("preserves spawn order within a primary group", () => {
    ...
    expect(groups.get("you@example.com")).toEqual(["multi", "primary-set"]);
  });
...
agents:
  alpha:
    auth:
      accounts: [you@example.com, ken+work@example.com]
`;
    yaml = appendAccountToAgent(yaml, "alpha", "alice@example.com");
    const groups = groupAgentsByPrimaryAccount(
      yaml,
      ["alpha"],
      "alice@example.com",
    );
    expect(groups.get("you@example.com")).toEqual(["alpha"]);
  });
});
